    ...
    <section class="order-detail">
        ...
        <div class="box-container">
          ...
          ?>
            <div class = "box">
              <div class = "col">
                  <p class = "title"><i class = "bi bi-calender-fill"> <?php $fetch_order['date'] ; ?></i></p>
                  <img src="image/<?php $fetch_product['image'] ;?>" class = "image">
                  <p class = "price"><?php $fetch_product['price']?> x <?php $fetch_order['qty'] ;?></p>
                  <h3 class = "name"><?php $fetch_product['name'] ;?></h3>
                  <p class = "grand_total">total amount payable : <span><?php  $grand_total ;?></span></p>
              </div>
              <div class = "col"></div>
                <p class="title">billing address</p>
                <p class = "user"><i class = "bi bi-person-pounding-box"><?php $fetch_order['name'] ;?></i></p>
                <p class = "user"><i class = "bi bi-phone"><?php $fetch_order['phone'] ;?></i></p>
                <p class = "user"><i class = "bi bi-envelope"><?php $fetch_order['email'] ;?></i></p>
                <p class = "user"><i class = "bi bi-pin-map-fill"><?php $fetch_order['address'] ;?></i></p>
                <p class = "title" >status</p>
                <p class = "status" style ="color:<?php if($fetch_order['status'] == 'delivered') {echo 'green' ;}
                elseif($fetch_order['status'] == 'canceled') {echo 'red' ;} 
                else{echo 'orange' ;}?>"><?php $fetch_order['status'] ;?></p>

                <?php if($fetch_order['status']=='canceled'){ ?>
                 <a href="checkout.php?get_id=<?php $fetch_product['id'] ; ?>" class = "btn">order again</a>
                <?php   }else{ ?>
                  <form  method="post">
                  <button type="submit" name="cancel" class = "btn" onclick = "return confirm('do you want to cancel')">cancel</button>
                  </form>
                <?php   }  ?>  
